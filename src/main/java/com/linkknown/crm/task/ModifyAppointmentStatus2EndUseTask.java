 ...
 */
@Component
public class ModifyAppointmentStatus2EndUseTask {
    ...
    public void modifyAppointmentStatus() {
        logger.info("==========>正在执行修改预约单状态为“已结束”定时任务");

        // 查看预约单的状态，如果某个单子已经过期了3个小时，系统自动将单子设置为“已结束”
        //1.查询所有“待处理”“进行中”的预约单
        ...
        //2.计算这些单子预约具体时间与当前时间的差值，找出过期3小时的单子集合
        ...
        logger.info("==========>已过期3小时,需要修改预约单状态为“已结束”的预约单ids:{}",needUpdateAppointmentIdList);

        //3.将这些单子设置为“已结束”
        String[] strIds = needUpdateAppointmentIdList.toArray(new String[0]);
        if (strIds.length>0){
            appointmentMapper.updateAppointmentStatusEndUseByBatchIds(strIds);
        }else{
